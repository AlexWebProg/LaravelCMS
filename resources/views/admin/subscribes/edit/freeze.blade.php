            ...
            <div class="col-12 col-lg-6">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">{{ ($subscribe->status === 'Заморожена') ? 'Разморозка' : 'Заморозка' }} подписки</h3>
                    </div>
                    <div class="card-body">
                        <input type="hidden" name="action_type" value="{{ ($subscribe->status === 'Заморожена') ? 'Разморозка' : 'Заморозка' }}"/>
                        <input type="hidden" name="status" value="{{ ($subscribe->status === 'Заморожена') ? 'Активна' : 'Заморожена' }}"/>
                        ...
                        @if ($subscribe->status === 'Заморожена')
                            <div class="form-group">
